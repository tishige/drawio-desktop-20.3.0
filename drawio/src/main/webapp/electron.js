...
const { Menu: menu, shell, dialog, clipboard, nativeImage, ipcMain, app, BrowserWindow } = require("electron");
...
const disableUpdate = require("./disableUpdate").disableUpdate() || process.env.DRAWIO_DISABLE_UPDATE === "true" || fs.existsSync("/.flatpak-info"); //This file indicates running in flatpak sandbox
...
app.on("ready", (e) => {
  ...
  try {
    program
      ...
      .option("-q, --quality <quality>", "output image quality for JPEG (default: 90)", parseInt)
      .option("-t, --transparent", "set transparent background for PNG")
      .option("-e, --embed-diagram", "includes a copy of the diagram (for PNG, SVG and PDF formats only)")
      .option("--embed-svg-images", "Embed Images in SVG file (for SVG format only)")
      .option("-b, --border <border>", "sets the border width around the diagram (default: 0)", parseInt)
      .option("-s, --scale <scale>", "scales the diagram size", parseFloat)
      .option("--width <width>", "fits the generated image/pdf into the specified width, preserves aspect ratio.", parseInt)
      .option("--height <height>", "fits the generated image/pdf into the specified height, preserves aspect ratio.", parseInt)
      .option("--crop", "crops PDF to diagram size")
      .option("-a, --all-pages", "export all pages (for PDF format only)")
      .option("-p, --page-index <pageIndex>", "selects a specific page, if not specified and the format is an image, the first page is selected", parseInt)
      .option("-g, --page-range <from>..<to>", "selects a page range (for PDF format only)", argsRange)
      .option("-u, --uncompressed", "Uncompressed XML output (for XML format only)")
      .option("-i, --csv", "CSV import") //tishige added csv import option
  ...
  if (options.export) {
                  ...
                  reply: function (msg, data) {
                    try {
                      if (data == null || data.length == 0) {
                        ...
                        if (outType != null) {
                          if (outType.isDir) {
                            outFileName = path.join(options.output, path.basename(curFile, path.extname(curFile))) + "." + format;
                          } else {
                        ...
                        try {
                          ...
                          if (program.rawArgs.indexOf("-k") > -1 || program.rawArgs.indexOf("--check") > -1) {
                            while (fs.existsSync(realFileName)) {
                              counter++;
                              realFileName = path.join(path.dirname(outFileName), path.basename(outFileName, path.extname(outFileName))) + "-" + counter + path.extname(outFileName);
                            }
                          }
    ...
    return;
  } else if (program.rawArgs.indexOf("-h") > -1 || program.rawArgs.indexOf("--help") > -1 || program.rawArgs.indexOf("-V") > -1 || program.rawArgs.indexOf("--version") > -1) {
              ...
              function processOneFile() {
                ...
                try {
                  ...
                  if (ext == ".csv") {
                    ...
                    win.webContents.send("args-obj", { args: program.args, csv: options.csv, csvdata: expArgs.xml, isMac: isMac });
                  }
                } catch (e) {
                  console.error("Error reading file: " + curFile);
                }
  ...
  if (isMac) {
    let template = [
        ...
        submenu: [
          {
            ...
            click() {
              shell.openExternal("https://www.diagrams.net");
            },
          },
          {
            label: "Support",
            click() {
              shell.openExternal("https://github.com/jgraph/drawio-desktop/issues");
            },
          },
          checkForUpdates,
          { type: "separator" },
      ...
      {
        label: "Edit",
        submenu: [{ role: "undo" }, { role: "redo" }, { type: "separator" }, { role: "cut" }, { role: "copy" }, { role: "paste" }, { role: "pasteAndMatchStyle" }, { role: "selectAll" }],
      },
    ];
...
function exportDiagram(event, args, directFinalize) {
  ...
  try {
    ...
    var pageByPage = args.format == "pdf" && !args.print,
      from,
      pdfs;
            ...
            function () {
              browser.capturePage().then(function (img) {
                ...
                if (args.embedXml == "1" && args.format == "png") {
                  data = writePngWithText(data, "mxGraphModel", args.xml, true, base64encoded);
                } else {
                  if (base64encoded) {
...
function checkFileContent(body, enc) {
  if (body != null) {
    ...
    if (cc1 == 0xfe && cc2 == 0xff) {
      if (cc3 == 0 && c4 == "<" && cc5 == 0 && c6 == "?" && cc7 == 0 && c8 == "x") {
        return true;
      }
    }
    ...
    if (cc1 == 0xff && cc2 == 0xfe) {
      if (c3 == "<" && cc4 == 0 && c5 == "?" && cc6 == 0 && c7 == "x" && cc8 == 0) {
        return true;
      }
    }
    ...
    if (cc1 == 0x00 && cc2 == 0x00 && cc3 == 0xfe && cc4 == 0xff) {
      if (cc5 == 0 && cc6 == 0 && cc7 == 0 && c8 == "<" && cc9 == 0 && cc10 == 0 && cc11 == 0 && c12 == "?" && cc13 == 0 && cc14 == 0 && cc15 == 0 && c16 == "x") {
        return true;
      }
    }
    ...
    if (cc1 == 0xff && cc2 == 0xfe && cc3 == 0x00 && cc4 == 0x00) {
      if (c5 == "<" && cc6 == 0 && cc7 == 0 && cc8 == 0 && c9 == "?" && cc10 == 0 && cc11 == 0 && cc12 == 0 && c13 == "x" && cc14 == 0 && cc15 == 0 && cc16 == 0) {
        return true;
      }
    }
    ...
    // image/png
    if (
      (cc1 == 137 && cc2 == 80 && cc3 == 78 && cc4 == 71 && cc5 == 13 && cc6 == 10 && cc7 == 26 && cc8 == 10) ||
      (cc1 == 194 &&
        cc2 == 137 &&
        cc3 == 80 &&
        cc4 == 78 &&
    ...
    if (cc1 == 0xff && cc2 == 0xd8 && cc3 == 0xff) {
       ...
       */
      if (cc4 == 0xe1 && c7 == "E" && c8 == "x" && c9 == "i" && c10 == "f" && cc11 == 0) {
        return true;
      }
    }
...
async function getFileDrafts(fileObject) {
  ...
  for (let i = 1; i < draftsPaths.length; i++) {
    try {
      ...
      drafts.push({ data: await fsProm.readFile(draftsPaths[i], "utf8"), created: stat.ctimeMs, modified: stat.mtimeMs, path: draftsPaths[i] });
    } catch (e) {} // Ignore
  }
...
async function saveFile(fileObject, data, origStat, overwrite, defEnc) {
  ...
  if (overwrite) {
    ...
    let stat = fs.existsSync(fileObject.path) ? await fsProm.stat(fileObject.path) : null;
...
// tishige
function cfvdrawiopath(fileName) {
  var path = "./flow/" + fileName + ".drawio";
...
function clipboardAction(method, data) {
  if (method == "writeText") {
    ...
    clipboard.write({ image: nativeImage.createFromDataURL(data.dataUrl), html: '<img src="' + data.dataUrl + '" width="' + data.w + '" height="' + data.h + '">' });
  }
}
  ...
  try {
    ...
    switch (args.action) {
      ...
      case "getCurDir":
        ret = await getCurDir();
        break;
      case "cfvdrawiopath": //tishige
        ret = await cfvdrawiopath(args.fileName);
        break;

      case "cfvclose": //tishige
        ret = await cfvclose();
