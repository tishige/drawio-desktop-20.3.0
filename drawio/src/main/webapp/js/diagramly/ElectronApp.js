...
(async function () {
  ...
  PrintDialog.electronPrint = function (editorUi, allPages, pagesFrom, pagesTo, fit, sheetsAcross, sheetsDown, zoom, pageScale, pageFormat) {
    var xml = "",
      title = "";
              ...
              menu.addItem(
                ...
                function () {
                  ...
                  if (file != null && file.isModified()) {
                    editorUi.confirm(mxResources.get("allChangesLost"), null, doOpenRecent, mxResources.get("cancel"), mxResources.get("discardChanges"));
                  } else {
                    doOpenRecent();
                  }
                },
  ...
  EditorUi.prototype.init = function () {
      ...
      mxUtils.bind(this, async function () {
        if (editorUi.getCurrentFile() != null) {
          ...
          if (paths !== undefined && paths[0] != null) {
            ...
            var encoding = asImage || /\.pdf$/i.test(path) || /\.vsdx$/i.test(path) || /\.vssx$/i.test(path) ? "base64" : "utf-8";
                  ...
                  try {
                    if (editorUi.isLucidChartData(data)) {
                      ...
                      if (asImage) {
                        ...
                        img.onload = function () {
                          editorUi.resizeImage(
                            ...
                            function (data2, w, h) {
                              ...
                              graph.setSelectionCell(graph.insertVertex(null, null, "", pt.x, pt.y, w, h, "shape=image;aspect=fixed;image=" + editorUi.convertDataUri(data2) + ";"));
                            },
                            true
                          );
                        };
                        ...
                        img.src = format == "svg" ? Editor.createSvgDataUri(data) : "data:image/" + format + ";base64," + data;
                      } else {
                        editorUi.spinner.stop();
    ...
    editorUi.updatePasteActionStates = function () {
      ...
      paste.setEnabled(this.editor.graph.cellEditor.isContentEditing() || (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())));
  ...
  App.prototype.loadArgs = function (argsObj) {
    ...
    if (argsObj.csv != null) {
      ...
      var text = argsObj.csvdata;

      //tishige import and save
      this.importCsv(text, null);
      this.save(null, success, error, null, false);
    }

    // tishige add else
    // If a file is passed, and it is not an argument (has a leading -)
    else if (paths !== undefined && paths[0] != null && paths[0].indexOf("-") != 0 && this.spinner.spin(document.body, mxResources.get("loading"))) {
      var path = paths[0];
      this.hideDialog();
          ...
          listener: mxUtils.bind(this, function (curr, prev) {
            ...
            if (curr.mtimeMs != prev.mtimeMs && !file.inConflictState) {
              ...
              file.addConflictStatus(
                mxUtils.bind(this, function () {
                  file.ui.editor.setStatus(mxUtils.htmlEntities(mxResources.get("updatingDocument")));
  ...
  App.prototype.pickFile = function () {
    ...
    if (file != null && file.isModified()) {
      this.confirm(mxResources.get("allChangesLost"), null, doPickFile, mxResources.get("cancel"), mxResources.get("discardChanges"));
    } else {
      doPickFile();
    }
  };
  ...
  LocalFile.prototype.getFilename = function () {
    ...
    if (filename.length > 0 && !/(\.xml)$/i.test(filename) && !/(\.html)$/i.test(filename) && !/(\.svg)$/i.test(filename) && !/(\.png)$/i.test(filename) && !/(\.drawio)$/i.test(filename)) {
      filename += ".drawio";
    }
  ...
  LocalFile.prototype.saveAs = async function (title, success, error) {
    ...
    var cfvidx = savetitle.lastIndexOf(".");

    if (cfvidx > 0) {
      cfvext = savetitle.substring(cfvidx + 1);
      cfvname = savetitle.substring(0, cfvidx);
    }

    var path = await requestSync({ action: "cfvdrawiopath", fileName: cfvname });
    if (path != null) {
      localStorage.setItem(".lastSaveDir", await requestSync({ action: "dirname", path: path }));
      this.fileObject = new Object();
      this.fileObject.path = path;
      ...
      // tishige added close method
      await requestSync({ action: "cfvclose" });
    }
  };
  ...
  App.prototype.createFileSystemFilters = function (defaultExt) {
    ...
    for (var i = 0; i < this.editor.diagramFileTypes.length; i++) {
      var obj = {
        name: mxResources.get(this.editor.diagramFileTypes[i].description) + " (." + this.editor.diagramFileTypes[i].extension + ")",
  ...
  EditorUi.prototype.createDownloadRequest = function (filename, format, ignoreSelection, base64, transparent, currentPage, scale, border, grid, includeXml) {
    var graph = this.editor.graph;
  ...
  ExportDialog.exportFile = function (editorUi, name, format, bg, s, b, dpi) {
    ...
    if (format == "xml" || format == "svg") {
      ...
      if ((format == "png" || format == "jpg" || format == "jpeg") && editorUi.isExportToCanvas()) {
        if (format == "png") {
          editorUi.exportImage(s, bg == null || bg == "none", true, false, false, b, true, false, null, null, dpi);
        } else {
